...
urlpatterns = patterns('blog.views',
    url(r'^(?P<year>\d{4})/(?P<month>\d{1,2})/(?P<pk>\d+)-(?P<slug>[-\w]+)/$', blog.views.ArticleView.as_view(), name='article-detail'),
    ...
    url(r'^(?P<section>[-\w]+)/$', blog.views.ArticleListView.as_view(), name="article-list-by-section"),
